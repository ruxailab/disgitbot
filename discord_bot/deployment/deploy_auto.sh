...
main() {
    ...
    # --- TEMPORARY DEPLOY TO GET URL ---
    print_step "Doing temporary deploy to get Cloud Run URL..."
    ...
    SERVICE_URL=$(echo "$DEPLOY_OUTPUT" | grep -oE 'https://[^ ]+\.run\.app')


    if [ -n "$SERVICE_URL" ]; then
    print_success "✅ Real Cloud Run URL: $SERVICE_URL"
    sed -i.bak "s|^OAUTH_BASE_URL=.*|OAUTH_BASE_URL=$SERVICE_URL|" "$ENV_PATH"
    else
    print_error "❌ Failed to get Cloud Run URL"
    exit 1
    fi

    # --- REDEPLOY WITH FULL CONFIGURATION ---
    print_step "Redeploying with correct OAUTH_BASE_URL..."

    print_success "Deployment complete!"
    echo -e "\n${CYAN}[STEP]${NC} Updating .env with service URL for OAUTH_BASE_URL..."
    if grep -q "^OAUTH_BASE_URL=" "$ENV_PATH"; then
        sed -i.bak "s|^OAUTH_BASE_URL=.*|OAUTH_BASE_URL=$SERVICE_URL|" "$ENV_PATH"
    else
        echo "OAUTH_BASE_URL=$SERVICE_URL" >> "$ENV_PATH"
    fi
    print_success "Updated OAUTH_BASE_URL in .env to: $SERVICE_URL"

    print_step "Re-deploying with correct OAUTH_BASE_URL..."
    # Recalculate ENV_VARS
    ENV_VARS=""
    echo "" >> "$ENV_PATH"
    while IFS= read -r line || [[ -n "$line" ]]; do
        if [[ ! $line =~ ^# && -n $line ]]; then
    ...
    # Deploy again with updated OAUTH_BASE_URL
    ...
    print_success "Second deployment with OAUTH_BASE_URL complete!"
